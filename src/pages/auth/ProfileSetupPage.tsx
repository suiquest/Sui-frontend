...
import { useTusky } from '../../hooks/useTusky'; // Your Tusky hook
...
const UnifiedOnboardingPage: React.FC = () => {
  ...
  const { isAuthenticated, signIn } = useTusky();
  ...
  useEffect(() => {
    ...
    if (savedProfile.name && savedProfile.role && savedProfile.walletAddress) {
      addLog('User already has complete profile', 'localStorage.getItem("userProfile")');
  ...
  const handleNext = async () => {
    ...
    if (currentStep === 3) {
      ...
      try {
        ...
        // Upload profile image to Tusky (placeholder)
        addLog('Uploading profile image to vault...', 'placeholder upload');
        const walrusBlobId = `placeholder_blob_${Date.now()}`;
        addLog('Profile image uploaded', `Blob ID: ${walrusBlobId}`);
        ...
        addLog('Executing transaction...', 'signAndExecuteTransaction');
        ...
        <div className="flex-1 max-w-md">
          ...
          <div className="bg-amber-900/20 border border-amber-700 rounded-lg p-4 mb-6 animate-pulse">
            <p className="text-amber-200 text-sm">
              Add skills to match with relevant bounties.
            </p>
          </div>
      ...
      <div className="max-w-6xl mx-auto p-8">
        ...
        <BottomActions
          ...
          nextLabel={
            ...
            currentStep === 2 ? "Next to Profile" : loading ? "Registering..." : "Complete Setup"
          }
          showSkip={currentStep > 1}
          showBack={currentStep > 1}
        />
        <div className="mt-8 bg-slate-800 p-4 rounded-lg max-h-64 overflow-y-auto">
          <h3 className="text-white text-lg font-medium mb-4">Logs</h3>
          {logs.map((log, index) => (
